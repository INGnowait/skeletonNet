...
class ConvPnPNet(nn.Module):
    def __init__(
        ...
        drop_prob=0.0,
        dropblock_size=5,
        skeleton_attention_type="concat",
        mask_attention_type="concat",
    ):
        """
        Args:
        ...
        self.skeleton_attention_type = skeleton_attention_type
        # -----------------------------------
    ...
    def forward(self, coor_feat, extents=None, skeleton_attention=None, mask_attention=None):
        """
        Args:
        ...
        #     x = coor_feat
        x = coor_feat
        if self.skeleton_attention_type != "none":
            assert skeleton_attention is not None
            if self.skeleton_attention_type == "mul":
                x = x * skeleton_attention
            elif self.skeleton_attention_type == "concat":
                x = torch.cat([x, skeleton_attention], dim=1)
            else:
                raise ValueError(f"Wrong mask attention type: {self.skeleton_attention_type}")
