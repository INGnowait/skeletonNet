...
from .datasets import air_flight #, lm_syn_imgn, lm_dataset_d2, lm_pbr, lm_blender, ycbv_pbr, ycbv_d2, ycbv_bop_test


cur_dir = osp.dirname(osp.abspath(__file__))
# from lib.utils.utils import iprint
__all__ = ["register_dataset", "register_datasets", "register_datasets_in_cfg", "get_available_datasets"]
# _DSET_MOD_NAMES = ["lm_syn_imgn", "lm_dataset_d2", "lm_pbr", "lm_blender", "ycbv_pbr", "ycbv_d2", "ycbv_bop_test", "air_flight"]
_DSET_MOD_NAMES = ["air_flight",]
...
def register_datasets_in_cfg(cfg):
    for split in ["TRAIN", "TEST", "SS_TRAIN", "TEST_DEBUG", "TRAIN_REAL", "TRAIN2", "TRAIN_SYN_SUP"]:
        for name in cfg.DATASETS.get(split, []):
            if name in DatasetCatalog.list(): #return all register datasets
                continue
            registered = False
