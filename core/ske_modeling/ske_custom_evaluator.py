...
class ske_EvaluatorCustom(DatasetEvaluator):
    ...
    def __init__(self, cfg, dataset_name, distributed, output_dir, train_objs=None):
        ...
        if 'air' in dataset_name:
            vertex_scale = self.data_ref.vertex_scale
            self.model_paths = [
                osp.join(self.data_ref.model_eval_dir, obj_namid, 'models', "{}.json".format(obj_namid)) for obj_namid in self.obj_names
            ]
            self.diameters = [self.data_ref.diameters[self.data_ref.objects.index(obj_name)] for obj_name in
                              self.obj_names]
            models_3D=[]
            for model_path in self.model_paths:
                model=mmcv.load(model_path)
                model['pts']=(np.array(model['pts'])*vertex_scale).reshape(3,-1).T
                models_3D.append(model)
            self.models_3d=models_3D
            # self.models_3d = [mmcv.load(model_path) for model_path in self.model_paths
            # ]
        else:
            self.model_paths = [
    ...
    def process(self, inputs, outputs, out_dict):
        ...
        for i, (_input, output) in enumerate(zip(inputs, outputs)):
            ...
            for inst_i in range(len(_input["roi_img"])):
                ...
                if cfg.MODEL.CDPN.NAME=='SKEN':
                    scene_im_id_split = _input["obj_scene_im_id"][inst_i].split("/")
                    ...
                    if cls_name is None:
                        continue
                    obj_id = int(scene_im_id_split[0])
                    scene_id = int(scene_im_id_split[1])
                    im_id = int(scene_im_id_split[2])
                    ...
                    if cfg.DEBUG:  # visualize pose
                        ...
                        save_path = osp.join(
                            cfg.OUTPUT_DIR, "vis", "{}_{}_{}_{:06d}_no_bbox.png".format(cls_name, obj_id, scene_id, im_id)
                        )
                        ...
                        save_path = osp.join(
                            cfg.OUTPUT_DIR, "vis", "{}_{}_{}_{:06d}_gt_est.png".format(cls_name, obj_id, scene_id, im_id)
                        )
                        mmcv.mkdir_or_exist(osp.dirname(save_path))
                        ...
                        grid_show(
                            [ren_im[:, :, ::-1], im_vis[:, :, ::-1]],
                            [f"ren_im_{cls_name}", f"{obj_id}/{scene_id}/{im_id}_{score}"],
                            row=1,
                            col=2,
                        )
                else:
    ...
    def get_gts(self):
        ...
        for im_dict in tqdm(dataset_dicts):
            file_name = osp.join(osp.normpath(osp.join(osp.dirname(osp.abspath(__file__)), "../../")), im_dict["file_name"])  # Relative path error, change to absolute path
    ...
    def _eval_predictions(self):
        ...
        if osp.exists(cache_path) and self.use_cache:
            ...
            if hasattr(self, "_predictions"):
                mmcv.dump(self._predictions, cache_path)  # save as cache_path
            else:
                raise RuntimeError("Please run inference first")

        recalls = OrderedDict()
        errors = OrderedDict()
        recalls_scene= OrderedDict()
        errors_scene = OrderedDict()
        self.get_gts()
        ...
        for scen in range(1, cfg.NUM_SCENE+1):
            recalls_scene[str(scen)] = OrderedDict()
            for metric_name in metric_names:
                recalls_scene[str(scen)][metric_name] = []
            errors_scene[str(scen)] = OrderedDict()
            for err_name in error_names:
                errors_scene[str(scen)][err_name] = []
        ...
        for obj_name in self.gts:
            ...
            for file_name, gt_anno in obj_gts.items():
                ...
                t_pred = obj_preds[file_name]["t"] * norm_scale   # 单位为m

                R_gt = gt_anno["R"]
                t_gt = gt_anno["t"] * norm_scale   # 单位为m

                t_error = te(t_pred, t_gt)
                scene = str(int(file_name.split('/')[-2]))
                ...
                if obj_name in cfg.DATASETS.SYM_OBJS:
                    ...
                    proj_2d_error = arp_2d(
                        R_pred, t_pred, R_gt_sym, t_gt, pts=self.models_3d[cur_label]["pts"], K=gt_anno["K"]  # self.models_3d的单位为m
                    )
                    ...
                    )  # 3d点在图像坐标系中的2d坐标之间的误差K*(Rx+t)/z

                    ad_error = add(
                        R_pred, t_pred, R_gt, t_gt, pts=self.models_3d[self.obj_names.index(obj_name)]["pts"]
                    )  # 3d点在相机坐标系下的3d坐标之间的误差（Rx+t）

                if "sken" not in method_name:
                    #########
                    ...
                    ############
                    recalls[obj_name]["ad_2"].append(float(ad_error < 0.02 * self.diameters[cur_label]))   # diameters的单位为m
                    ...
                    #########
                    errors[obj_name]["ad"].append(ad_error)  # 单位为m
                    errors[obj_name]["re"].append(r_error)
                    errors[obj_name]["te"].append(t_error)  # 单位为m
                    errors[obj_name]["proj"].append(proj_2d_error)
                    ############
                    ...
                    recalls[obj_name]["ad_10"].append(float(ad_error < 0.1 * self.diameters[cur_label]))
                    # deg, cm
                    recalls[obj_name]["rete_2"].append(float(r_error < 2 and t_error < 2))
                    recalls[obj_name]["rete_5"].append(float(r_error < 5 and t_error < 5))
                    recalls[obj_name]["rete_10"].append(float(r_error < 10 and t_error < 10))

                    recalls[obj_name]["re_2"].append(float(r_error < 2))
                    recalls[obj_name]["re_5"].append(float(r_error < 5))
                    recalls[obj_name]["re_10"].append(float(r_error < 10))

                    recalls[obj_name]["te_2"].append(float(t_error < 2))
                    recalls[obj_name]["te_5"].append(float(t_error < 5))
                    recalls[obj_name]["te_10"].append(float(t_error < 10))
                    # px
                    recalls[obj_name]["proj_2"].append(float(proj_2d_error < 2))
                    recalls[obj_name]["proj_5"].append(float(proj_2d_error < 5))
                    recalls[obj_name]["proj_10"].append(float(proj_2d_error < 10))
                    ...
                    #TODO:按场景分类
                    errors_scene[scene]["ad"].append(ad_error)
                    errors_scene[scene]["re"].append(r_error)
                    errors_scene[scene]["te"].append(t_error)
                    errors_scene[scene]["proj"].append(proj_2d_error)
                    ############
                    recalls_scene[scene]["ad_2"].append(float(ad_error < 0.02 * self.diameters[cur_label]))  # diameters的单位为m
                    recalls_scene[scene]["ad_5"].append(float(ad_error < 0.05 * self.diameters[cur_label]))
                    recalls_scene[scene]["ad_10"].append(float(ad_error < 0.1 * self.diameters[cur_label]))
                    # deg, cm
                    recalls_scene[scene]["rete_2"].append(float(r_error < 2 and t_error < 2))
                    recalls_scene[scene]["rete_5"].append(float(r_error < 5 and t_error < 5))
                    recalls_scene[scene]["rete_10"].append(float(r_error < 10 and t_error < 1))

                    recalls_scene[scene]["re_2"].append(float(r_error < 2))
                    recalls_scene[scene]["re_5"].append(float(r_error < 5))
                    recalls_scene[scene]["re_10"].append(float(r_error < 10))

                    recalls_scene[scene]["te_2"].append(float(t_error < 2))
                    recalls_scene[scene]["te_5"].append(float(t_error < 5))
                    recalls_scene[scene]["te_10"].append(float(t_error < 1))
                    # px
                    recalls_scene[scene]["proj_2"].append(float(proj_2d_error < 2))
                    recalls_scene[scene]["proj_5"].append(float(proj_2d_error < 5))
                    recalls_scene[scene]["proj_10"].append(float(proj_2d_error < 10))


        # summarize
        obj_names = sorted(list(recalls.keys()))
        scene_names = sorted(list(recalls_scene.keys()))

        header = ["objects"] + obj_names +  [f"Avg({len(obj_names)})"] + scene_names
        big_tab = [header]
        for metric_name in metric_names:
            line = [metric_name]
            this_line_res = []
            ...
            if "sken" in method_name:
                for scen_id in scene_names:
                    res = recalls_scene[scen_id][metric_name]
                    if len(res) > 0:
                        line.append(f"{100 * np.mean(res):.2f}")
                        this_line_res.append(np.mean(res))
                    else:
        ...
        for error_name in ["re", "te"]:
            ...
            if "sken" in method_name:
                for scen_id in scene_names:
                    res = errors_scene[scen_id][error_name]
                    if len(res) > 0:
                        line.append(f"{np.mean(res):.2f}")
                        this_line_res.append(np.mean(res))
                    else:
