...
def filter_invalid_in_dataset_dicts(dataset_dicts, visib_thr=0.0):
    ...
    new_dicts = []
    for dataset_dict in dataset_dicts:
        new_dict = {_k: _v for _k, _v in dataset_dict.items() if _k not in ["annotations"]}  # _k是dict中的key _v是key对应的值
        if "annotations" in dataset_dict:
            new_annos = []
            for inst_id, anno in enumerate(dataset_dict["annotations"]):
...
def filter_empty_dets(dataset_dicts) -> object:
    """
...
def load_detections_into_dataset(
    ...
    if "air" not in dataset_name:
        ...
        for record in dataset_dicts:
            obj_scene_im_id = record["obj_scene_im_id"]
            dets_i = detections[obj_scene_im_id]
            ...
            for obj, cur_annos in obj_annotations.items():
                scores = [ann["score"] for ann in cur_annos]
                sel_annos = [ann for _, ann in
                             sorted(zip(scores, cur_annos), key=lambda pair: pair[0], reverse=True)][
                            :top_k_per_obj
                            ]
                annotations.extend(sel_annos)
            # NOTE: maybe [], no detections
